  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return Scaffold(
          appBar: AppBar(title: const Text('Studio vocal hausa TTS')),
          body: SafeArea(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(20, 8, 20, 28),
              children: <Widget>[
                const _IntroCard(),
                if (controller.vocabularyUpdateMessage != null) ...<Widget>[
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          Expanded(
                            child: Text(controller.vocabularyUpdateMessage!),
                          ),
                        ],
                      ),
                    ),
                        ...
                        OutlinedButton.icon(
                          onPressed: () async {
                            ...
                            if (context.mounted) {
                              _message(
                                context,
                                'Le vocabulaire embarqué a été restauré.',
                              );
                            }
                          },
                          icon: const Icon(Icons.offline_pin_outlined),
                          label: const Text('Restaurer le vocabulaire embarqué'),
                        ),
                      ],
                    ),
                  ),
      ...
      if (action == _ResumeAction.restart) {
        final confirmed = await showDialog<bool>(
          ...
          builder: (dialogContext) => AlertDialog(
            ...
            content: const Text(
              'Les masters WAV de cette voix seront supprimés du téléphone.',
            ),
            actions: <Widget>[
              TextButton(
        ...
        children: <Widget>[
          ...
          Expanded(
            child: Text(
              ...
              'Les masters TTS sont captés en WAV 24 kHz sans traitement, '
              'entièrement hors-ligne. La campagne ASR séparée reste à 16 kHz.',
            ),
          ),
        ],
      ),
...
class _RecordingScreenState extends State<RecordingScreen> {
  ...
  final List<double> _meterPeaks = <double>[];
  double _instantaneousPeak = 0;
  double _instantaneousRms = 0;
  ...
  void initState() {
    ...
    amplitudeSubscription = widget.audio
  ...
  void _onAmplitude(Amplitude amplitude) {
    if (!recording || !mounted || !amplitude.current.isFinite) {
      return;
    }
    final peak = math
        ...
        .toDouble()
        .clamp(0.0, 1.0);
    _meterPeaks.add(peak);
    final calibrationDb = widget.controller.meterCalibrationDb(speaker) ?? -9.0;
    final calibration = math.pow(10, calibrationDb / 20).toDouble();
    setState(() {
      _instantaneousPeak = peak;
      _instantaneousRms = (peak * calibration).clamp(0.0, 1.0);
    });
  }
  ...
  Future<void> _finish({
    ...
    try {
      ...
      final meterMeanPeak = _meterPeaks.isEmpty
          ? null
          : _meterPeaks.reduce((a, b) => a + b) / _meterPeaks.length;
      final meterMaxPeak = _meterPeaks.isEmpty
          ? null
          : _meterPeaks.reduce(math.max);
      ...
      final driftDb = widget.controller.levelDriftDb(speaker, prompt);
      if (mounted &&
          driftDb != null &&
          driftDb.abs() > AppController.maximumLevelDriftDb) {
        await _offerRetakeForDrift(driftDb);
      }
    } on Object catch (error) {
      await widget.audio.cancel();
      if (mounted) {
  ...
  Future<void> _offerRetakeForDrift(double driftDb) async {
    final direction = driftDb > 0 ? 'plus forte' : 'plus faible';
    final retake = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        icon: const Icon(Icons.multiline_chart),
        title: const Text('Niveau de voix différent'),
        content: Text(
          'Cette prise est ${driftDb.abs().toStringAsFixed(1)} dB '
          '$direction que la moyenne de la séance. Pour garder une voix TTS '
          'stable, il est conseillé de la refaire à la même distance du micro.',
        ),
        actions: <Widget>[
          TextButton(
            onPressed: () => Navigator.pop(dialogContext, false),
          ...
          FilledButton(
            onPressed: () => Navigator.pop(dialogContext, true),
            child: const Text('Refaire maintenant'),
          ),
        ],
      ),
    );
    if (retake == true && mounted) {
      _message(
        context,
        'Maintenez le bouton et réenregistrez cette même phrase.',
      );
    }
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      _VoiceLevelMeter(
                        recording: recording,
                        instantaneousRms: _instantaneousRms,
                        instantaneousPeak: _instantaneousPeak,
                        sessionAverageRms: widget.controller.sessionAverageRms(
                          speaker,
                        ),
                        calibrationSamples: widget.controller
                            .calibrationSampleCount(speaker),
                        calibrationDb: widget.controller.meterCalibrationDb(
                          speaker,
                        ),
                      ),
                      const SizedBox(height: 18),
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            ...
            'RMS ${take.rms.toStringAsFixed(3)} · '
            'pic ${take.peak.toStringAsFixed(3)} · '
...
class _VoiceLevelMeter extends StatelessWidget {
  const _VoiceLevelMeter({
    required this.recording,
    required this.instantaneousRms,
    required this.instantaneousPeak,
    required this.sessionAverageRms,
    required this.calibrationSamples,
    required this.calibrationDb,
  });

  static const maximumScale = 0.80;

  final bool recording;
  final double instantaneousRms;
  final double instantaneousPeak;
  final double? sessionAverageRms;
  final int calibrationSamples;
  final double? calibrationDb;

  double _fraction(double value) => (value / maximumScale).clamp(0.0, 1.0);

  @override
  Widget build(BuildContext context) {
    final displayedRms = recording ? instantaneousRms : 0.0;
    final inTarget =
        displayedRms >= AppController.targetRmsMin &&
        displayedRms <= AppController.targetRmsMax;
    final color = instantaneousPeak > AppController.targetPeakMax
        ? Theme.of(context).colorScheme.error
        : inTarget
        ? const Color(0xff16803c)
        : const Color(0xffd97706);
    final average = sessionAverageRms;

    return Semantics(
      label:
          'Niveau de voix. Cible RMS de 0,10 à 0,20. '
          'Pic maximal 0,70.',
      child: Container(
        width: double.infinity,
        padding: const EdgeInsets.all(14),
        ...
        child: Column(
          ...
          children: <Widget>[
                  ...
                  child: SizedBox(
                    ...
                    child: Stack(
                      children: <Widget>[
                        ...
                        Positioned(
                          left: width * _fraction(AppController.targetRmsMin),
                          width:
                              width *
                              (_fraction(AppController.targetRmsMax) -
                                  _fraction(AppController.targetRmsMin)),
                          top: 0,
                          bottom: 0,
                          child: ColoredBox(
                            color: const Color(
                        ...
                        Positioned(
                          left: 0,
                          width: width * _fraction(displayedRms),
                          top: 6,
                          bottom: 6,
                        ...
                        Positioned(
                          left:
                              width * _fraction(AppController.targetPeakMax) -
                              1,
                          width: 2,
                          top: 0,
                          bottom: 0,
                          child: const ColoredBox(color: Color(0xffb3261e)),
                        ),
                        if (average != null)
                          Positioned(
                            left: width * _fraction(average) - 2,
                            width: 4,
                            top: 0,
                            bottom: 0,
            ...
            Text(
              recording
                  ? 'RMS estimé ${instantaneousRms.toStringAsFixed(3)} · '
                        'pic ${instantaneousPeak.toStringAsFixed(3)}'
                  : 'Maintenez le bouton pour afficher le niveau instantané',
              style: const TextStyle(fontWeight: FontWeight.w700),
            ),
            Text(
              average == null
                  ? 'Moyenne de séance : en construction'
                  : 'Repère noir : moyenne de séance '
                        '${average.toStringAsFixed(3)}',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            Text(
              calibrationSamples == 0
                  ? 'Calibration initiale : ajustement au premier WAV'
                  : 'Calibré sur $calibrationSamples prise(s) '
                        '(${calibrationDb!.toStringAsFixed(1)} dB)',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 5),
            const Text(
              'Bande verte : RMS 0,10–0,20 · trait rouge : pic 0,70 · '
              'repère noir : moyenne',
    ...
    try {
      ...
      await SharePlus.instance.share(
        ShareParams(
          ...
          subject: 'Dataset TTS hausa — ${speaker.name}',
          text:
              'Masters WAV 24 kHz de ${speaker.name} (${speaker.code}) '
              'pour Piper TTS',
        ),
      );
    } on Object catch (error) {
