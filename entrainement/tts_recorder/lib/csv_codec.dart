...
List<Prompt> parsePromptsCsv(String source) {
  ...
  const expected = <String>['id', 'texte_hausa', 'affichage'];
  if (rows.first.length != expected.length ||
      List.generate(
        expected.length,
        (i) => rows.first[i].trim() == expected[i],
      ).contains(false)) {
    throw const FormatException('En-tête attendu : id,texte_hausa,affichage');
  }
  ...
  for (var index = 1; index < rows.length; index++) {
    ...
    final id = row[0].trim();
    final hausa = row[1].trim();
    final display = row[2].trim();
    if (id.isEmpty || hausa.isEmpty || display.isEmpty) {
    ...
    prompts.add(Prompt(id: id, hausaText: hausa, display: display));
  }
  if (prompts.isEmpty) {
...
String piperMetadataRow(String fileName, String hausaText) {
