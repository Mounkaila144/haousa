...
def main() -> None:
    ...
    inset = square.crop((margin, margin, square.width - margin, square.height - margin)).convert(
        "RGB"
    )
    ...
    for folder, size in LAUNCHER_SIZES.items():
        ...
        scaled = (
            ...
            else mark.resize((round(mark.width * safe_px / mark.height), safe_px), Image.LANCZOS)
        )
        foreground.paste(
            scaled,
