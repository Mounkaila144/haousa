...
import 'package:hausa_mobile/correction/correction_controller.dart';
...
/// hausa canonique (générée par le moteur via l'API) et envoie la correction en
...
class _CorrectionScreenState extends ConsumerState<CorrectionScreen> {
  ...
  Future<void> _submit(HausaGeneration generation) async {
    ...
    Navigator.of(context).pushReplacementNamed(
      ...
      arguments: ConfirmedResult(
        ...
        hausaText: generation.hausaText,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final HausaGeneration? ready = correction.ready;
...
/// Affiche l'état de génération de la forme hausa (idle/chargement/donnée/erreur).
class _GenerationView extends StatelessWidget {
  const _GenerationView({required this.generation});

  final AsyncValue<HausaGeneration>? generation;

  @override
  Widget build(BuildContext context) {
    final AsyncValue<HausaGeneration>? value = generation;
    if (value == null) {
      return Text(
    ...
    return value.when(
      ...
      data: (HausaGeneration data) => Semantics(
        liveRegion: true,
        label: 'Nombre ${data.number}. En hausa : ${data.hausaText}.',
        child: ExcludeSemantics(
          child: Theme(
            ...
            child: HausaNumberDisplay(number: data.number),
          ),
        ),
      ),
      error: (Object error, _) {
        final String message = error is GenerationFailure
            ? error.message
            : 'Impossible de générer la forme hausa pour le moment.';
        return Semantics(
          liveRegion: true,
          label: message,
          child: Text(
