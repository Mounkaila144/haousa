...
class _RecordingScreenState extends ConsumerState<RecordingScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      unawaited(ref.read(recordingControllerProvider.notifier).onAppResumed());
    }
  }
  ...
  Future<void> _openProcessing(
    ...
    await Navigator.of(
      context,
    ).pushNamed(AppRoutes.processing, arguments: handoff);
    if (!mounted) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (recording.phase == RecordingPhase.ready &&
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          unawaited(_openProcessing(controller, recording.handoff!));
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    switch (state.phase) {
      ...
      case RecordingPhase.recording:
        return _ActiveRecording(state: state, onStop: onStop);
