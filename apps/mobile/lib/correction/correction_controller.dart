...
/// asynchrone de sa forme hausa. `generation == null` signifie « pas encore de
...
class CorrectionState {
  ...
  /// Une correction n'est envoyable que si une forme hausa valide est prête.
  HausaGeneration? get ready => generation?.valueOrNull;
}
...
/// forme hausa. Aucune règle numérique n'est recalculée localement.
class CorrectionController extends StateNotifier<CorrectionState> {
  ...
  // Doit rester synchronisé avec `MAX_VALUE` de `hausa_numbers` (extension
  // million, story au-delà de 1.7) : 99 999 millions + un reste complet.
  static const int maxValue = 99999999999;

  final HausaGeneratorRepository _repository;
  ...
  void onInputChanged(String raw) {
    ...
    if (number == null || number < 0 || number > maxValue) {
      state = CorrectionState(
        input: digits,
        generation: const AsyncError<HausaGeneration>(
          GenerationFailure(
    ...
    state = CorrectionState(
      input: digits,
      generation: const AsyncLoading<HausaGeneration>(),
    );
  ...
  Future<void> _generate(int number) async {
    ...
    _cancelToken = token;
    try {
      final HausaGeneration generation = await _repository.generate(
        number: number,
        cancelToken: token,
      );
      ...
      state = CorrectionState(
        input: state.input,
        generation: AsyncData<HausaGeneration>(generation),
      );
      ...
      state = CorrectionState(
        input: state.input,
        generation: AsyncError<HausaGeneration>(failure, StackTrace.empty),
      );
    }
  }
...
final correctionControllerProvider =
    StateNotifierProvider.autoDispose<CorrectionController, CorrectionState>((
      ...
      return CorrectionController(
        repository: ref.watch(hausaGeneratorRepositoryProvider),
