...
Uint8List pcmFromWav(Uint8List bytes) {
  ...
  while (offset + 8 <= bytes.length) {
    ...
    if (id == 'fmt ') {
      ...
      final int end = (body + size <= bytes.length)
          ? body + size
          : bytes.length;
...
Uint8List wavFromPcm(Uint8List pcm, {int sampleRate = kSampleRate}) {
  const int headerSize = 44;
  final int byteRate = sampleRate * kChannels * (kBitsPerSample ~/ 8);
...
/// Convertit le signal flottant Sherpa-ONNX en WAV PCM16 mono jouable.
Uint8List wavFromFloatSamples(Float32List samples, {required int sampleRate}) {
  final Uint8List pcm = Uint8List(samples.length * 2);
  final ByteData output = ByteData.sublistView(pcm);
  for (int index = 0; index < samples.length; index++) {
    final double clamped = samples[index].clamp(-1.0, 1.0).toDouble();
    final int value = clamped < 0
        ? (clamped * 32768).round()
        : (clamped * 32767).round();
    output.setInt16(index * 2, value, Endian.little);
  }
