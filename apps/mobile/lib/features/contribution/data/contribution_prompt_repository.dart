...
import 'package:hausa_mobile/features/contribution/data/contribution_prompt_catalog.dart';
...
class ContributionPromptFailure implements Exception {
  const ContributionPromptFailure({required this.type, required this.message});
...
class ApiContributionPromptRepository implements ContributionPromptRepository {
  ...
  final HausaGeneratorRepository _generator;
  ...
  Future<ContributionPrompt> nextPrompt({
    ...
    try {
      final HausaGeneration generation = await _generator.generate(
        number: seed.number,
        cancelToken: cancelToken,
      );
      return ContributionPrompt(
        expectedNumber: generation.number,
        expectedPrompt: generation.hausaText,
...
final contributionPromptRepositoryProvider =
    Provider<ContributionPromptRepository>((ref) {
      return ApiContributionPromptRepository(
        ...
        generator: ref.watch(hausaGeneratorRepositoryProvider),
      );
    });
