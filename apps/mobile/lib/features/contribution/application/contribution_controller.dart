...
class ContributionController extends StateNotifier<ContributionState> {
  ...
  void requestWithdrawal() {
    ...
    state = _withWithdrawal(phase: ContributionWithdrawalPhase.confirmation);
  }
  ...
  Future<void> _cancelFlow() async {
    ...
    if (mounted) {
      state = ContributionState(recording: _recordingController.state);
    }
  }
  ...
  Future<void> _prefetchPromptAfterSuccess(ContributionReceipt receipt) async {
    _promptCancelToken?.cancel('prompt_replaced');
  ...
  Future<void> _withdrawContributions() async {
    ...
    try {
      ...
      await _withdrawalRepository.withdraw(anonId: _anonId, cancelToken: token);
      if (!mounted) {
        return;
      }
      state = _withWithdrawal(
  ...
  Future<void> _serialize(Future<void> Function() action) {
    ...
    _operation = next.then<void>((_) {}, onError: (_, __) {});
    return next;
  }
...
final contributionControllerProvider =
    ...
    >((ref) {
      final ContributionController controller = ContributionController(
        ...
        withdrawalRepository: ref.watch(
          contributionWithdrawalRepositoryProvider,
        ),
        ...
        onWithdrawalCompleted: ref
            .read(consentStatusProvider.notifier)
            .clearAcceptanceAfterWithdrawal,
