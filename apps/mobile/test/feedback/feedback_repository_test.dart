...
void main() {
  ...
  test(
    'poste le body confirmed exact sans versions et valide le receipt',
    () async {
  ...
  test(
    'poste le body repeat_requested avec proposed_number nullable',
    () async {
      stubResponse(
    ...
    await expectLater(
      ...
      throwsA(
        isA<FeedbackFailure>()
            ...
            .having(
              (FeedbackFailure e) => e.code,
              'code',
              'RECOGNITION_NOT_FOUND',
            )
            .having((FeedbackFailure e) => e.requestId, 'request', 'req-404')
            .having(
              (FeedbackFailure e) => e.message,
              'message',
  ...
  final Map<int, FeedbackFailureType> statusMappings =
      <int, FeedbackFailureType>{
  ...
  test('mappe timeout, absence réseau, certificat et annulation', () async {
    ...
    await verifyType(DioExceptionType.cancel, FeedbackFailureType.cancelled);
  });
}
