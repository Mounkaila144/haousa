...
import 'package:hausa_mobile/features/contribution/presentation/contribution_route.dart';
...
class _UnusedPromptRepository implements ContributionPromptRepository {
  @override
  Future<ContributionPrompt> nextPrompt({required CancelToken cancelToken}) {
    throw UnimplementedError();
  }
}
...
class _WidgetContributionController extends ContributionController {
  ...
  static ContributionPrompt _prompt(int number) {
    return ContributionPrompt(
      ...
      expectedPrompt: number == 100 ? 'ɗari' : 'ɗari biyu da biyar',
...
void main() {
  ...
  testWidgets('affiche le prompt, enregistre, envoie puis continue', (
    ...
    expect(find.text('ɗari'), findsOneWidget);
    expect(
    ...
    final Finder stopButton = find.byKey(
      const Key('stop-contribution-recording-button'),
    );
  ...
  testWidgets('une erreur de prompt propose Réessayer puis récupère', (
    ...
    expect(find.byKey(const Key('contribution-prompt-error')), findsOneWidget);
    await tester.tap(find.byKey(const Key('retry-contribution-prompt-button')));
    await tester.pump();
  ...
  testWidgets('affiche la progression et une erreur d’envoi actionnable', (
    ...
    await tester.pump();
    expect(find.byKey(const Key('contribution-upload-error')), findsOneWidget);
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      final RecordingController recording = _recordingController();
      final _FixedConsentController consent = _FixedConsentController(
        _validConsent(),
      );
      final _WidgetContributionController contribution =
          _WidgetContributionController(
      ...
      expect(find.textContaining('audios supprimés'), findsOneWidget);
      semantics.dispose();
    },
  );
