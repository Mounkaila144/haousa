...
void main() {
  test('le catalogue couvre la plage, les échelles et la confusion ASR', () {
    ...
    expect(
      contributionPromptCatalog.map(
        (ContributionPromptSeed seed) => seed.number,
      ),
    ...
    final List<ContributionPromptSeed> confusionSeeds =
        contributionPromptCatalog
            ...
            .toList();
    // 100 est résolu côté API en une forme contenant « ɗari », terme
    // canonique visé par la paire acoustique zangu→ɗari du lexique.
    expect(
      confusionSeeds.map((ContributionPromptSeed seed) => seed.number),
      contains(100),
    );
  });
