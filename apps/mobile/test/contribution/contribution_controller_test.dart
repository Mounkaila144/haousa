...
import 'package:hausa_mobile/features/contribution/application/contribution_controller.dart';
import 'package:hausa_mobile/features/contribution/data/consent_repository.dart';
import 'package:hausa_mobile/features/contribution/data/contribution_metadata_source.dart';
import 'package:hausa_mobile/features/contribution/data/contribution_prompt_repository.dart';
import 'package:hausa_mobile/features/contribution/data/contribution_upload_repository.dart';
import 'package:hausa_mobile/features/contribution/data/contribution_withdrawal_repository.dart';
import 'package:hausa_mobile/features/contribution/models/contribution_models.dart';
...
void main() {
  ...
  test(
    'charge le prompt avant la capture et assemble toutes les métadonnées',
    () async {
      controller.dispose();
      controller = createController(
      ...
      expect(pending.expectedPrompt, 'ɗari 100');
  ...
  test(
    'les métadonnées optionnelles nulles ne bloquent pas le brouillon',
    () async {
      await controller.loadNextPrompt();
      await controller.startRecording();
      await controller.stopRecording();
  ...
  test(
    'changement de prompt et perte de consentement nettoient le temporaire',
    () async {
      await controller.loadNextPrompt();
      await controller.startRecording();
      await controller.stopRecording();
  ...
  test(
    'envoie le brouillon, nettoie le WAV et précharge le prompt suivant',
    () async {
      await controller.loadNextPrompt();
      await controller.startRecording();
      await controller.stopRecording();
  ...
  test(
    'une erreur d’envoi est actionnable et nettoie toujours le WAV',
    () async {
      controller.dispose();
      uploadRepository = _FakeUploadRepository(
  ...
  test(
    'annule immédiatement une requête en cours puis nettoie le flux',
    () async {
      controller.dispose();
      uploadRepository = _FakeUploadRepository(
  ...
  test('exige une confirmation avant tout retrait', () async {
    ...
    expect(controller.state.withdrawalPhase, ContributionWithdrawalPhase.idle);
    expect(withdrawalRepository.calls, 0);
  });
  ...
  test(
    'le retrait confirmé annule un upload en cours avant son appel',
    () async {
      controller.dispose();
      uploadRepository = _FakeUploadRepository(
        handler: (_, CancelToken token) async {
  ...
  test('une erreur de retrait est réessayable', () async {
    ...
    await controller.confirmWithdrawal();
    expect(controller.state.withdrawalPhase, ContributionWithdrawalPhase.error);
