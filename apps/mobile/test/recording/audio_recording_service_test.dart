...
void main() {
  ...
  test(
    'configure WAV PCM16 mono 16 kHz et un chemin temporaire unique',
    () async {
      ...
      final List<dynamic> calls = verify(
        () => recorder.start(captureAny(), path: captureAny(named: 'path')),
      ).captured;
  ...
  test('borne physiquement un WAV à dix secondes', () async {
    ...
    await file.writeAsBytes(_wavBytes(duration: const Duration(seconds: 11)));
