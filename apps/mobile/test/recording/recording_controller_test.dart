...
void main() {
  ...
  test(
    'permission refusée explique le besoin et permet de réessayer',
    () async {
      when(
        () => permissionGateway.check(),
      ).thenAnswer((_) async => MicrophonePermissionStatus.denied);
      when(
        () => permissionGateway.request(),
      ).thenAnswer((_) async => MicrophonePermissionStatus.denied);
  ...
  test('refus permanent ouvre les réglages et recontrôle au retour', () async {
    when(
      () => permissionGateway.check(),
    ).thenAnswer((_) async => MicrophonePermissionStatus.permanentlyDenied);

    await controller.start();

    expect(controller.state.phase, RecordingPhase.permissionPermanentlyDenied);
    verifyNever(() => permissionGateway.request());
    verifyNever(() => service.start());
  ...
  test(
    'refuse les durées sous 1 seconde et au-dessus de 10 secondes',
    () async {
      when(() => service.inspect(any())).thenAnswer(
        (_) async => const AudioFileInspection(
          exists: true,
  ...
  test(
    'la suppression post-envoi simulé est explicite et idempotente',
    () async {
      await controller.start();
      await controller.stop();
