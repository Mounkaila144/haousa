...
import 'package:hausa_mobile/correction/hausa_generator_repository.dart';
import 'package:hausa_mobile/feedback/feedback_models.dart';
import 'package:hausa_mobile/feedback/feedback_repository.dart';
import 'package:hausa_mobile/models/recognition_result.dart';
import 'package:hausa_mobile/navigation/app_routes.dart';
import 'package:hausa_mobile/widgets/number_keypad.dart';
import 'package:hausa_mobile/widgets/hausa_number_display.dart';

class FakeGeneratorRepository implements HausaGeneratorRepository {
  FakeGeneratorRepository(this.handler);

  final Future<HausaGeneration> Function(int number) handler;
...
class MockGeneratorRepository extends Mock
    implements HausaGeneratorRepository {}
...
const String _hausa235 = 'ɗari biyu da talatin da biyar';
...
void main() {
  ...
  testWidgets('saisie 235 génère la forme hausa sans l’afficher', (
    WidgetTester tester,
  ) async {
    final FakeGeneratorRepository generator = FakeGeneratorRepository(
      (int n) async => HausaGeneration(
        number: n,
        hausaText: _hausa235,
    ...
    expect(find.text(_hausa235), findsNothing);
    expect(
      find.descendant(
        of: find.byType(HausaNumberDisplay),
  ...
  testWidgets('un nombre hors plage est refusé sans appel API ni envoi', (
    ...
    final FakeGeneratorRepository generator = FakeGeneratorRepository(
      (int n) async =>
          HausaGeneration(number: n, hausaText: 'x', grammarVersion: '1.1.0'),
    );
  ...
  testWidgets('une erreur de génération API affiche un message clair', (
    ...
    expect(
      find.text('Ce nombre n’a pas encore de forme hausa disponible.'),
      findsOneWidget,
    );
    final FilledButton submit = tester.widget(
      find.byKey(const Key('submit-correction-button')),
...
Future<void> _pumpCorrection(
  WidgetTester tester, {
  required HausaGeneratorRepository generator,
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: <Override>[
        hausaGeneratorRepositoryProvider.overrideWithValue(generator),
...
RecognitionResult _result() {
  return const RecognitionResult(
    id: 'rec-id',
    recognizedNumber: 42,
    hausaText: 'ashirin da biyu',
    normalizedText: 'ashirin da biyu',
    confidence: 0.6,
