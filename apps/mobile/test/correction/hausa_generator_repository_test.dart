...
void main() {
  late MockDio dio;
  late DioHausaGeneratorRepository repository;

  setUp(() {
    dio = MockDio();
    repository = DioHausaGeneratorRepository(dio);
  });

  test('appelle le générateur API et valide son contrat versionné', () async {
    when(
      () => dio.get<dynamic>(any(), cancelToken: any(named: 'cancelToken')),
    ).thenAnswer(
      (_) async => Response<dynamic>(
        ...
        data: <String, dynamic>{
          'number': 235,
          'hausa_text': 'ɗari biyu da talatin da biyar',
    ...
    final HausaGeneration result = await repository.generate(
      number: 235,
      cancelToken: token,
    );
