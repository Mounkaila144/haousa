...
import 'package:hausa_mobile/screens/confirmation_screen.dart';
...
void main() {
  ...
  testWidgets('sélection d’une alternative confirme ce nombre', (
    ...
    expect(find.text('bakwai'), findsNothing);
  });
  ...
  testWidgets('un double tap ne déclenche qu’un seul envoi', (
    ...
    final RecordingFeedbackRepository repository = RecordingFeedbackRepository(
      (FeedbackRequest r) => pending.future,
    );
    ...
    expect(repository.requests, hasLength(1));
    expect(
      find.byKey(const Key('feedback-progress-indicator')),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.byKey(const Key('confirm-expression-button')),
        findsOneWidget,
      );
...
Future<void> _pumpConfirmation(
    ...
    ProviderScope(
      ...
      child: MaterialApp(
        ...
        routes: <String, WidgetBuilder>{
          AppRoutes.recording: (_) =>
              const Scaffold(key: Key('recording-screen')),
        },
      ),
    ),
  );
  // Laisse Riverpod construire les dépendances avant d'ouvrir l'écran qui
  // démarre sa boucle de parole dès sa première construction.
  await tester.pump();
  ...
  navigator.push<void>(
    PageRouteBuilder<void>(
      ...
      pageBuilder: (_, __, ___) => ConfirmationScreen(result: result),
    ),
  );
  if (played == null) {
    await tester.pumpAndSettle();
...
RecognitionResult _confirmResult() {
  return const RecognitionResult(
    ...
    alternatives: <RecognitionAlternative>[
      ...
      RecognitionAlternative(number: 7, hausaText: 'bakwai', score: 0.4),
      RecognitionAlternative(number: null, hausaText: 'bruit', score: 0.1),
    ],
    modelVersion: 'mock',
    grammarVersion: 'v1',
  );
...
RecognitionResult _confirmWithoutCandidate() {
  return const RecognitionResult(
    ...
    decision: Decision.confirm,
    alternatives: <RecognitionAlternative>[
      RecognitionAlternative(number: null, hausaText: 'bruit', score: 0.2),
    ],
    modelVersion: 'mock',
    grammarVersion: 'v1',
  );
