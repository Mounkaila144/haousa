# Application mobile Hausa

Socle Flutter Android de l'application Hausa. Cette fondation utilise Material 3,
...
ASR_MODE=mock ASR_MOCK_TEXT="ashirin da uku a ƙara goma sha biyar" \
...
## Synthèse vocale locale M3

La restitution utilise `sherpa_onnx` 1.13.4 et le modèle Piper
`adab-tech/murya-piper-hausa-tts`, embarqué sous
`assets/models/hausa_tts/`. Elle ne fait aucun appel HTTP et n'envoie aucun
texte à l'API. L'ASR reste, lui, inchangé et peut continuer à fonctionner sur le
serveur.

Au premier énoncé, les assets ONNX sont copiés dans le stockage privé de
l'application, puis le modèle est chargé dans un isolate persistant. Les
calculs suivants réutilisent la même instance. La voix est toujours recherchée
dans `speaker_id_map` sous le nom `M3`; l'identifiant actuellement validé est 1.
Une voix absente ou un identifiant différent provoque une erreur explicite,
sans repli vers un autre locuteur.

Le modèle est graphémique (`phoneme_type: text`). Son frontend Sherpa est donc
`characters`, alimenté par `tokens.txt`; aucun `espeak-ng-data` et aucun lexique
phonétique ne sont requis. Le mobile met le texte en minuscules et Unicode NFD,
et refuse les chiffres : le résultat canonique en mots hausa fourni par l'API
est la seule forme acceptée avant synthèse.

La provenance, la révision, les empreintes et la procédure de conversion sont
documentées dans `assets/models/hausa_tts/README.md`. Pour régénérer les
ressources à partir du modèle amont validé :
...
5. Annuler une prise, simuler la suppression post-envoi, puis confirmer avec
   `adb shell run-as ne.hausa.hausa_mobile` qu'aucun
   `hausa_recording_*.wav` ne reste dans le cache.
...
`adb shell run-as ne.hausa.hausa_mobile` sans extraire ni journaliser l'audio.
...
tri, aucun recalcul de confiance ou de forme hausa côté mobile. Les scores,
...
nombre ou sans forme hausa est ignorée. Si aucune proposition numérique
...
La saisie manuelle (`corrected`), la génération hausa et l'historique sont hors
