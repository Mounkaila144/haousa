...
def score(asr: AsrResult, settings: Settings | None = None):
    normalized = hausa_numbers.normalize(asr.text)
...
class TestCompositeConfidence:
    def test_each_signal_is_exposed_and_bounded(self) -> None:
        canonical = hausa_numbers.generate(100)
        asr = make_asr(
            canonical,
            acoustic_score=0.8,
            candidates=[
                Candidate(number=None, text=hausa_numbers.generate(90), score=0.6),
            ],
        )
    ...
    def test_unknown_tokens_lower_variant_signal(self) -> None:
        result = score(make_asr(f"{hausa_numbers.generate(100)} unknown"))
        assert result.variant == 0.5
        assert result.grammatical == 0.0

    def test_single_numeric_candidate_has_full_margin(self) -> None:
        result = score(make_asr(hausa_numbers.generate(1)))
        assert result.margin == 1.0

    def test_normalized_hausa_alias_is_not_penalized_as_confusion(self) -> None:
        asr = make_asr(
            "ɗari",
            candidates=[Candidate(number=None, text="dari", score=0.85)],
        )
        result = score(asr)

        assert result.confusion == 1.0
        assert hausa_numbers.parse("dari") == 100
    ...
    def test_configured_weights_change_score(self) -> None:
        ...
        result = score(make_asr(hausa_numbers.generate(1), acoustic_score=0.25), settings)
...
class TestPolicy:
    settings = Settings()
    one = [NumericCandidate(number=1, text="ɗaya", score=0.9)]
    ambiguous = [
        NumericCandidate(number=1, text="ɗaya", score=0.80),
        NumericCandidate(number=2, text="biyu", score=0.75),
    ]
...
class TestConfidencePolicyIntegration:
    def test_high_confidence_canonical_hundred_is_accepted(
        self, mock_recognizer: MockRecognizer
    ) -> None:
        mock_recognizer.set_text("ɗari", acoustic_score=0.95)
    ...
    def test_close_candidates_are_ordered_and_confirmed(
        ...
        mock_recognizer.set_text(hausa_numbers.generate(100), acoustic_score=0.80)
        mock_recognizer.set_candidates(
            [
                Candidate(number=None, text=hausa_numbers.generate(90), score=0.75),
                Candidate(number=None, text=hausa_numbers.generate(100), score=0.80),
            ]
        )
    ...
    def test_settings_override_can_lower_accept_threshold(
        ...
        mock_recognizer.set_text(hausa_numbers.generate(1), acoustic_score=0.30)
