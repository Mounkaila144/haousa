...
déploiement, l'API non. Le décodeur émettait `sau` / `a ƙara`, que
...
def _transcribe_transport(grammar_version: str | None) -> httpx.MockTransport:
    payload = {
        ...
        "candidates": [],
        "latency_ms": 12,
        "model_version": "hausa_w2v2_modele",
    }
...
def test_versions_identiques_aucune_derive(settings: Settings) -> None:
    current = hausa_numbers.load_lexicon().grammar_version
...
def test_version_differente_signale_une_derive(settings: Settings) -> None:
    ...
    assert mismatch.api_grammar_version == hausa_numbers.load_lexicon().grammar_version
