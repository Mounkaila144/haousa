...
class NumericCandidate:
    """Candidat ASR dont le texte est parsable comme nombre hausa."""
...
def _variant_signal(text: str) -> float:
    ...
    Le balayage est **par phrase, pas par token** : les operateurs Hausa peuvent
    etre des locutions de plusieurs mots (`a ƙara`, `a raba chi sau`). On
    apparie donc la forme connue la plus longue a chaque position.
    """
    trace = hausa_numbers.normalize_with_trace(text)
    tokens = trace.normalized.split()
    if not tokens:
        return 0.0

    lexicon = hausa_numbers.load_lexicon()
...
def is_known_asr_confusion(left: str, right: str, confusions: dict[str, str]) -> bool:
    ...
    left_tokens = hausa_numbers.normalize_with_trace(left).normalized.split()
    right_tokens = hausa_numbers.normalize_with_trace(right).normalized.split()
...
def _confusion_signal(asr: AsrResult) -> float:
    ...
    confusions = hausa_numbers.load_lexicon().asr_confusions
    return (
        0.0
        if any(
