...
Le décodeur contraint (service ASR) et l'analyseur (``hausa_numbers`` côté API)
...
def api_grammar_version() -> str:
    ...
    """
    return hausa_numbers.load_lexicon().grammar_version
    ...
    if asr_grammar_version not in _reported:
        ...
        log.error(
            ...
            remediation=(
                ...
                "(systemctl restart hausa-api hausa-asr)."
            ),
        )
    return mismatch
...
async def probe(
    ...
    *,
    ...
    dans ``journalctl -u hausa-api`` dans les secondes qui suivent un déploiement,
