...
``grammar_version`` **provient du lexique** (``hausa_numbers.load_lexicon()``),
...
class GrammarGenerateResponse(BaseModel):
    """Forme hausa canonique d'un nombre, versionnée par le lexique."""
...
@router.get(
    ...
    summary="Forme hausa canonique d'un nombre",
    responses={
...
def grammar_generate(
    ...
    """Génère la forme hausa via ``hausa_numbers`` (source unique de vérité).
