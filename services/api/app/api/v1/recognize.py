...
class RecognizedExpression(BaseModel):
    ...
    operator: Literal["+", "-", "*", "/"]
    right: int
    #: Forme à **prononcer**, quand elle diffère de ``hausa_text``.
    #:
    #: Variante eventuelle destinee a la synthese vocale.
    spoken_text: str = ""
    #: Forme hausa canonique de l'opération entendue (relisible à voix haute).
    hausa_text: str
...
async def recognize(
    ...
    try:
        ...
        async with asyncio.timeout(settings.ASR_TIMEOUT_SECONDS):
            ...
            expected_prompt = (
                response.normalized_text.strip() or response.hausa_text.strip() or "[à revoir]"
            )
