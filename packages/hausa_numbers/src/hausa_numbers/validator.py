...
déterministe convertit nombre ↔ hausa sans perte, indépendamment de l'ASR.

Portée de la preuve :

- Plage ``0``–``1 000 000`` : l'invariant DOIT tenir ; toute violation est une
  **erreur** (fait échouer la CI).
- Au-delà (grandes echelles de ``miliyan`` a ``tiriliyan``), ``MAX_VALUE``
  atteint ``999 999 999 999 999``, mais une preuve
...
Exécuter : ``uv run python -m hausa_numbers.validator`` (ou ``make invariant``).
Pour auditer une plage précise au-delà de ``RESOLVED_MAX`` : ``uv run python -m
hausa_numbers.validator <start> <end>``.
"""
...
#: Au-dela, le meme parseur recursif est verifie par echantillonnage.
