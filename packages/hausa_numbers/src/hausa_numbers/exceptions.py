"""Exceptions du paquet ``hausa_numbers``.
...
class GenerationError(Exception):
    """Erreur de base du générateur ``nombre → hausa``.
...
class ParseError(Exception):
    """Échec d'analyse d'un texte hausa vers un nombre.
...
class ExpressionParseError(ParseError):
    """Échec d'analyse d'une **expression** hausa (story 6.1).
