...
confusion sont dérivées du moteur `hausa_numbers` (source unique).
...
- **Split par locuteur** : fonction pure de `(seed, speaker_key)` — seed
  documentée `hausa-benchmark-split-v1`. Un locuteur ne peut jamais apparaître
...
Le décodage CTC est restreint à la **grammaire des nombres hausa** : la sortie est
...
#    lourd : torch + transformers, cf. HAUSA_ASR_SETUP.md).
...
- **Signal** : `confidence = exp(-(nll_contraint − nll_libre) / nb_tokens)` —
  ...
  par trame, une courte insertion dans un long silence peut paraître anodine ;
  la normalisation par token évite de diluer ce coût acoustique.
...
- **Corrections de noms justifiables uniquement** : faute de frappe mécanique
  (`biyarwav` → `biyar`) ou **variante déclarée au lexique** (`dari` → `ɗari`).
