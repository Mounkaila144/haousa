...
from hausa_numbers.expressions import Expression, render_expression
...
def lexicon_without(tmp_path: Path, names: Sequence[str]) -> Path:
    ...
    """
    source = Path(hausa_numbers.__file__).parent / "lexicon.yaml"
...
def test_an_unresolved_operator_is_never_planned(tmp_path):
    ...
    lexicon = hausa_numbers.load_lexicon(lexicon_without(tmp_path, ["divide"]))
...
def test_targets_include_out_of_domain_cases():
    ...
    assert "RESULT_OUT_OF_RANGE" in refusals
...
def test_a_number_heard_instead_of_an_expression_fails():
    result = _evaluate({"a.wav": "23+15"}, {"a.wav": hausa_numbers.generate(38)})
    assert result.exact_expression_accuracy == 0.0
...
def test_number_only_report_still_mentions_the_metric_at_zero():
    ...
    result = evaluate(
        entries,
        asr_source_for({"n.wav": hausa_numbers.generate(42)}),
        get_settings(),
    )
