...
# Lexique de test : rend 2 et 3 mutuellement confusables (biyu ↔ uku).
LEXICON = dataclasses.replace(hausa_numbers.load_lexicon(), asr_confusions={"biyu": "uku"})
...
def test_pure_pipeline_exact_and_reject() -> None:
    exact = run_recognition_pipeline(_asr(hausa_numbers.generate(5)), SETTINGS)
...
def _mini_corpus() -> tuple[list[ManifestEntry], dict[str, AsrResult]]:
    entries = [
        ...
        _entry("b.wav", 100, condition="bruit"),  # exact (long + confusion via ɗari? non ici)
        _entry("c.wav", 2, condition="calme"),  # confusion : prédit 3
        _entry("d.wav", 7, condition="bruit"),  # rejet (texte non numérique)
    ]
    hypotheses = {
        "a.wav": _hyp(hausa_numbers.generate(5), latency_ms=40),
        "b.wav": _hyp(hausa_numbers.generate(100), latency_ms=60),
        "c.wav": _hyp(hausa_numbers.generate(3), latency_ms=80),  # uku → 3 (faux)
...
def test_confusion_matrix_marks_known_close_pair() -> None:
    ...
    assert pairs[(2, 3)].close is True  # biyu ↔ uku dans asr_confusions
...
def test_hypotheses_roundtrip(tmp_path: Path) -> None:
    ...
    rows = [asr_result_to_dict("a.wav", _asr(hausa_numbers.generate(5)))]
    path.write_text("\n".join(json.dumps(r) for r in rows) + "\n", encoding="utf-8")
    loaded = load_hypotheses(path)
    assert loaded["a.wav"].text == hausa_numbers.generate(5)
