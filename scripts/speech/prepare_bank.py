...
3. **correction des noms de fichier** évidents : ``biyuwav.wav`` → ``biyu.wav``
...
from hausa_numbers.loader import load_lexicon  # noqa: E402
...
def canonical_name(stem: str, vocabulary: set[str]) -> tuple[str, bool]:
    ...
    1. le suffixe ``wav`` collé au mot (``biyuwav`` → ``biyu``), faute de frappe
    ...
    2. une **variante linguistique** déclarée au lexique ramenée à sa forme
       canonique (``dari`` → ``ɗari``). Ce n'est pas une devinette : c'est la
