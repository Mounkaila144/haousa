"""Restitution vocale hausa par concaténation — cœur réutilisable (story 6.1, task 6).
...
- **Source unique.** Les formes viennent de ``hausa_numbers`` (``generate``,
...
from hausa_numbers.expressions import Expression, ExpressionResult, render_expression, render_result
...
# --------------------------------------------------------------------------- #
# Ce qu'il faut prononcer (composé depuis hausa_numbers, jamais réécrit)
# --------------------------------------------------------------------------- #
...
def result_utterance(result: ExpressionResult) -> tuple[Segment, ...]:
    """Segments prononçant le résultat — reste de division inclus (``saura``)."""
...
def target_vocabulary() -> set[str]:
    ...
    (milliers, ``da``, million) — la grammaire étant compositionnelle, cela
    suffit à énumérer le vocabulaire complet. S'y ajoutent :

    - le marqueur de reste de division (``saura``), sans lequel aucun résultat de
