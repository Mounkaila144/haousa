...
def test_conversion_produces_the_canonical_format(tmp_path):
    source = tmp_path / "raw" / "ashirin.wav"
    write_source(source, tone(0.5))

    prepare(source.parent, tmp_path / "out")

    with wave.open(str(tmp_path / "out" / "words" / "ashirin.wav"), "rb") as reader:
...
def test_trimming_can_be_disabled(tmp_path):
    ...
    def duration(root: Path) -> float:
        with wave.open(str(root / "words" / "ashirin.wav"), "rb") as reader:
...
@pytest.mark.parametrize(
    ("stem", "expected"),
    [("biyuwav", "biyu"), ("hamsinwav", "hamsin"), ("tamaninwav", "tamanin")],
)
...
def test_a_correct_name_is_untouched():
    assert canonical_name("ashirin", VOCABULARY) == ("ashirin", False)


def test_a_declared_variant_is_resolved_to_its_canonical_form():
    """`dari` → `ɗari` : ce n'est pas une devinette, c'est la table du lexique."""
    assert canonical_name("dari", VOCABULARY) == ("ɗari", True)


def test_an_unknown_name_is_never_guessed():
    """Aucun rapprochement approximatif : un nom hors lexique reste tel quel."""
    assert canonical_name("ashirinn", VOCABULARY) == ("ashirinn", False)
    assert canonical_name("d arii", VOCABULARY) == ("d arii", False)


def test_unknown_files_are_reported_not_dropped_silently(tmp_path):
    source = tmp_path / "raw"
    write_source(source / "ashirin.wav", tone(0.3))
    ...
    assert "ashirin" in report.words
...
def test_prompts_are_separated_from_words(tmp_path):
    ...
    report = prepare(source, tmp_path / "out")
    assert set(report.words) == {"ashirin"}
...
def test_report_lists_what_is_still_missing(tmp_path):
    ...
    assert "ashirin" not in report.missing_words
    assert "sha" in report.missing_words
...
def test_an_unreadable_file_is_reported_not_swallowed(tmp_path):
    source = tmp_path / "raw"
    source.mkdir()
    (source / "ashirin.wav").write_bytes(b"pas du tout un wav")

    report = prepare(source, tmp_path / "out")
    assert [name for name, _ in report.failed] == ["ashirin.wav"]
    assert not report.ok
