...
import pytest
from hausa_numbers.expressions import Expression, evaluate, render_result
from hausa_numbers.generator import generate
...
# --------------------------------------------------------------------------- #
# Ce qui est prononcé vient de hausa_numbers, jamais réécrit
# --------------------------------------------------------------------------- #
...
def test_result_utterance_says_the_division_remainder():
    """« 20 reste 3 » doit être dit, marqueur `saura` compris."""
    result = evaluate(Expression(103, "/", 5))
    assert render_result(result) == "ashirin saura uku"
...
def test_expression_utterance_replays_the_operation():
    ...
    assert "ƙara" in words


def test_the_remainder_marker_is_part_of_the_target_vocabulary():
    """Sans `saura`, aucun résultat de division non entière n'est prononçable."""
    assert "saura" in target_vocabulary()
...
def test_resolved_operators_are_part_of_the_target_vocabulary():
    ...
    """
    from hausa_numbers.loader import load_lexicon
...
def test_missing_reports_each_absent_segment_once(tmp_path):
    ...
    utterance = (("word", "ashirin"), ("word", "ashirin"), (PROMPT, PROMPT_CONFIRM))
    assert bank.missing(utterance) == [("word", "ashirin"), (PROMPT, PROMPT_CONFIRM)]
...
def test_missing_lists_are_actionable(tmp_path):
    bank = _bank_covering(tmp_path, ["ashirin"], prompts=[PROMPT_CONFIRM])
    assert "ashirin" not in missing_vocabulary(bank)
